  ...
  try {
    ...
    for (const owner of eligible) {
      ...
      try {
        ...
        if (send.success) {
          sent++;
          // Weekly rollover — advance the opening_balance to the current
          // balance and pin opening_balance_date to today so next week's
          // report shows "Opening = current balance from last week" plus
          // the deltas since. Skipped on the very first successful send so
          // the initial report still anchors on the manually-reconciled
          // opening_balance; subsequent runs roll forward each week.
          if (isFirstSend) {
